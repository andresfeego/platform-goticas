...
class AgregarGaleria extends Component {
    ...
    guardar() {
        ...
        return new Promise((resolve, reject) => {
            ...
            request
            .post('/responseLIGDS/crearGaleria')
            .send({ nombre: this.state.nombre, descripcion: this.state.descripcion })
            .set('accept', 'json')
            .end((err, res) => {
                if (err) {
                    console.log(err);
    ...
    onSubmit = async () => {
        nuevoMensaje(tiposAlertas.cargando, "Creando Galería")
        this.validarInfo().then(() => {
            this.guardar().then(() => {
                this.props.fun.getGalerias()
    ...
    render() {
        return (
            ...
            <React.Fragment>
                ...
                >
                    <DialogTitle id="max-width-dialog-title"><div className="tituloAgregarActividad">Nuevo galería</div></DialogTitle>
                    <DialogContent>
