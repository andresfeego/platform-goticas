 ...
 class CambioPass extends Component {
    ...
    guardarSesionMAestro(maestro){
        request
                ...
                .send({pass: this.state.password1, id: maestro.correo})
                .set('accept', 'json')
                .end((err, res) => {
                        if (err) {
                            console.log(err);
    ...
    cambiarContrasena(){
        request
                .post('/responseLIGDS/profesor/cambiarContrasena')
                .send({pass: this.state.password1, id: this.props.maestroAdmin.id})
                .set('accept', 'json')
                .end((err, res) => {
                        if (err) {
                            console.log(err);
                            ...
                            this.guardarSesionMAestro(this.props.maestroAdmin)
