...
class AgregarRecursoRespuesta extends Component {
    ...
    guardarRecursoDB(){
        request
              .post('/responseLIGDS/agregarEvidencia')
              .send({tipo: this.state.tipo, link: this.state.link, idRespuesta: this.props.idRespuesta})
              .set('accept', 'json')
              .end((err, res) => {
                      if (err) {
                          console.log(err);
                          ...
                          nuevoMensaje(tiposAlertas.cargadoSuccess, "Se ha agregado la evidencia")
    ...
    render() {
        return (
            <React.Fragment>
                ...
                <Box boxShadow={6} className="button buttonDos" onClick={() => this.handleClickOpen()}>Agregar evidencia</Box>
                ...
                >
                    <DialogTitle id="max-width-dialog-title"><h2 className="tituloAgregarActividad">Nueva evidencia</h2></DialogTitle>
                    <DialogContent>
